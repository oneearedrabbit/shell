...
def test_safe_resolve():
    ...
    assert (
        safe_resolve(path, "../../abc/../test/fixtures")
        == f"{path}/test/fixtures"
    )
    assert (
        safe_resolve(path, "../../abc/../.test/fixtures")
        == f"{path}/.test/fixtures"
    ...
    assert safe_resolve("..", "../test") == str(
        PosixPath(f"{path}/../test").resolve()
    )
    assert (
        safe_resolve("/userland", "81039b9d-028d-404c-bb0b-adbe84870177")
        == "/userland/81039b9d-028d-404c-bb0b-adbe84870177"
    )
